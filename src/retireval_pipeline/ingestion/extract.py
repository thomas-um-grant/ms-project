...
def extract_documents(files_path: list[str]) -> list[dict]:
    ...
    for file_path in files_path:
        if file_path.endswith(".pdf"):
            ...
            raise Exception(
                f"The file ({file_path}) type is not supported. Documents can be .pdf, .word, or .txt",
            )
    ...
    for page_number, page in tqdm(enumerate(doc)):
        ...
        docs.append(
            Doc(
                ...
                doc_token_count=len(text)
                / 4,  # 1 token ~= 4 char: https://help.openai.com/en/articles/4936856-what-are-tokens-and-how-to-count-them
                text=text,
            ),
        )
